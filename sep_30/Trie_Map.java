...
class TrieNode<T> {
    // Map to store child nodes, where the key is the character and value is the
    // child TrieNode
    Map<Character, TrieNode<T>> children;

    // Value associated with this node (null if this node doesn't represent the end
    // of a word)
    T value;
...
class Trie<T> {
    ...
    public T search(String word) {
        ...
        // Return the value if the word exists and is marked as end of word, otherwise
        // null
    ...
    public List<String> getWordsWithPrefix(String prefix) {
        ...
        // If the prefix exists in the Trie, collect all words starting from its last
        // node
        if (prefixNode != null) {
...
public class Trie_Map {
    public static void main(String[] args) {
        ...
        System.out.println("Value for 'app': " + trie.search("app")); // Output: null

        // Demonstrate prefix checking
        System.out.println("Starts with 'app': " + trie.startsWith("app")); // Output: true
        System.out.println("Starts with 'ba': " + trie.startsWith("ba")); // Output: true
